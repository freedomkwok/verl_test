...
class OpenManusAgent:
    ...
    def _init_env_clients(self) -> List[Any]: # Renamed and return type changed
        ...
        ENV_TO_TASK_CLASS = {
            "gmail_calendar": "GmailCalendarTask"
        ...
        try:
            ...
            raise ImportError(f"Could not import Task class {task_class_name} from openmanus_rl.agentgym.agentenv.agentenv.envs: {e}")
        ...
        for i, port in enumerate(self.config.env_ports):
            ...
            try:
                ...
                task_instance = None
                if task_class_name == "GmailCalendarTask":
                    task_instance = TaskClass(client_args=client_args, tokenizer=self.tokenizer, n_clients=1, job_id=1) # -1 for no_dataset
                else:
                    task_instance = TaskClass(client_args=client_args, n_clients=1)

                if hasattr(task_instance, 'env_clients') and task_instance.env_clients:  ## if hasattr(task_instance, 'clients') and task_instance.clients:
                    client = task_instance.env_clients[0]
    ...
    def _run_single_rollout(self, initial_prompt_ids: torch.Tensor, task_idx: int, client: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            gpt_response_tag = self.tokenizer.encode('\nAssistant: ', add_special_tokens=False, return_tensors='pt')
            human_response_tag = self.tokenizer.encode('\nHuman: ', add_special_tokens=False, return_tensors='pt')

            response_limit = -1
            # --- Interaction Loop --- 
            for t in range(self.config.max_turns):
                if response_limit > self.config.max_response_limit:
                    break
                turns = t + 1
                ...
                generation_config = GenerationConfig(
                    ...
                    temperature=self.config.temperature, # Consider adjusting temperature/sampling based on validation vs training
                    do_sample=True
                )
                ...
                # Explicitly set do_sample to ensure it's not overridden
                padded_gen_input_proto.meta_info['do_sample'] = True
                ...
                print(f"[Agent._run_single_rollout][{task_idx}][Turn {t+1}] Response[{len(response_text)}]: {response_text}")
                ...
                # # Debug logging
                # print(f"[Agent._run_single_rollout][{task_idx}][Turn {t+1}] Extracted Action: {action_text}")
                # print(f"[Agent._run_single_rollout][{task_idx}][Turn {t+1}] Action Type: {action_types[0]}")

                # Execute environment step using the provided client
                # if action_text is None: action_text = ""
                step_output = client.step(response_text)
                next_obs_text = step_output.state
                # if "ERROR" in next_obs_text:
                #     raise ValueError(f"bad response: {next_obs_text}")
                ...
                if not done:
                    print(f"[Agent._run_single_rollout][{task_idx}][Turn {t+1}] Next Obs: {next_obs_text[:500]}...")
                    ...
                    prev_start = torch.where(current_input_ids[0] != self.tokenizer.pad_token_id)[0][0].item()
                    prev_end = current_input_ids.shape[1]
                    # Find the actual content length (excluding padding tokens)
                    current_input_ids = current_input_ids[:, prev_start:]
                    non_pad_mask = response_ids[0] != self.tokenizer.pad_token_id
                    response_ids_no_pad = response_ids[0, non_pad_mask].unsqueeze(0)
                    print("original length", prev_end - prev_start, "response_ids_no_pad len:", response_ids_no_pad.shape[1], "next_obs_ids len:", next_obs_ids.shape[1])
                    # Concatenate actual content with response_ids and next_obs_ids
                    current_input_ids = torch.cat([
                        current_input_ids.to(response_ids_no_pad.device), # Move to same device as response_ids
                        gpt_response_tag.to(response_ids_no_pad.device),
                        response_ids_no_pad,
                        human_response_tag.to(response_ids_no_pad.device),
                        next_obs_ids.to(response_ids_no_pad.device) # Move to same device
                    ], dim=1)

                    # Apply the same padding logic as the initial prompt
                    # Create attention mask for the concatenated sequence
                    temp_attention_mask = torch.ones_like(current_input_ids)

                    print("new length before padding", current_input_ids.shape[1])
                    response_limit = current_input_ids.shape[1]
                    # Apply the same postprocess_data function used for initial prompts
                    current_input_ids, temp_attention_mask = verl_F.postprocess_data(
                        {'input_ids': current_input_ids,  "attention_mask": temp_attention_mask},
                        temp_attention_mask,
                        max_length=self.config.max_prompt_length,
                        pad_token_id=self.tokenizer.pad_token_id,
                        left_pad=True,
                        truncation='left'  # Truncate from left to keep most recent content
                    )

                    print("new length after padding", current_input_ids.shape[1])
                else:
    ...
    def run_llm_loop(self, gen_batch: DataProto, output_dir: str = None, global_steps: int = 0) -> DataProto:
        ...
        # is_owners = gen_batch.non_tensor_batch["is_owner"]
        task_ids = gen_batch.non_tensor_batch["item_id"]
        ...
        for i in range(batch_size):
            task_idx = int(task_ids[i:i+1].item().replace("gmail_",""))
            gen_batch.meta_info['idx'] = task_idx
            initial_prompt = initial_prompts_ids[i:i+1]  # Keep batch dim
            # is_owner = bool(is_owners[i:i+1].item())  ##Customized Changes
            print(f"[Agent.run_llm_loop] Task {task_idx} is owner: {False}")
    ...
    def _convert_rollout_results_to_dataproto(self, results: List[Dict], original_batch: DataProto) -> DataProto:
        ...
        batch_prompt_masks = [] # Initialize batch_prompt_masks
        ...
        # elif isinstance(original_indices, int):
        #     original_indices = [original_indices]
        original_indices_map = {int(task_idx.replace("gmail_", "")): i for i, task_idx in enumerate(original_batch.non_tensor_batch["item_id"])}
        ...
        for result_dict in results:
            ...
            per_rollout_ground_truth.append(original_batch[original_batch_idx].non_tensor_batch['goal_type'])
            ...
            # --- Pad and Truncate --- 
            full_input_ids = torch.cat(conversation_ids_list, dim=1) # full_input_ids is gpt response
            full_info_mask = torch.cat(info_mask_parts, dim=1) #full_info_mask is human response and gpt response
            ...
            prompt_length = conversation_ids_list[0].shape[1]
            ...
            if seq_len > target_len:
                ...
                remaining_prompt_length = max(0, prompt_length - current_removed)
                ...
                prompt_masks = torch.zeros_like(full_input_ids)
                prompt_masks[:, 0:remaining_prompt_length] = 1
                seq_len = target_len
                ...
                prompt_masks = torch.zeros_like(full_input_ids)
                prompt_start = -seq_len
                prompt_end = prompt_length - seq_len
                prompt_masks[:, prompt_start:prompt_end] = 1
            ...
            batch_prompt_masks.append(prompt_masks)
            batch_info_mask.append(full_info_mask) # non-env info mask
            batch_token_level_rewards.append(token_level_rewards) # reward distribute over the response
        ...
        final_batch = {
            ...
            "prompt_masks": torch.cat(batch_prompt_masks, dim=0),
        ...
        final_meta_info['reward_model'] = {"ground_truth": per_rollout_ground_truth}
    ...
    def postprocess_predictions(self, predictions: List[Any]) -> Tuple[List[str], List[str]]:
        """
        Process predictions into actions and content based on Action/Action Input format.
        Extracts the action and formats it for environment consumption.
        ...
        Returns:
            Tuple of (action types list ['action' or 'response' or None],
                    action contents list [formatted action string or empty string])
        """
        actions = []
        contents = []
        ...
        for prediction in predictions:
            if isinstance(prediction, str):
                # First try XML-like tags (for backward compatibility)
                ...
                if action_match:
                    ...
                    # Try to extract Action/Action Input format
                    action_match = re.search(r'Action:\s*(\w+)', prediction, re.MULTILINE)
                    action_input_match = re.search(r'Action Input:\s*(\{.*?\})', prediction, re.DOTALL | re.MULTILINE)
                    
                    if action_match and action_input_match:
                        action_name = action_match.group(1).strip()
                        action_input = action_input_match.group(1).strip()
                        # Format as single line for environment
                        formatted_action = f"Action:{action_name} with Action Input:{action_input}"
                        actions.append('action')
                        contents.append(formatted_action)
                    elif action_match:
                        # Only action name found
                        action_name = action_match.group(1).strip()
                        actions.append('action')
                        contents.append(f"Action:{action_name}")
                    else:
                        # If no recognized format, assume it's neither a specific action nor response
                        actions.append(None)
