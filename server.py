...
mkdir -p /data/models && mkdir -p /workspace/OpenRL2 && cd /workspace/OpenRL2 && mkdir -p /output/logs/ && mkdir -p /data/verl_checkpoints && \
...
mkdir -p /workspace/OpenRL && cd /workspace/OpenRL && git init && git remote add origin https://$GH_TOKEN@github.com/freedomkwok/OpenRL.git && git fetch --all  && git checkout -b main origin/main && git pull && \
